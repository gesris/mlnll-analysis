...
def main(args):
    ...
    plt.plot(x_r_sys_nosysimpl, f_deltaNLL_sys_nosysimpl(x_r_sys_nosysimpl), color='C3', lw=linewidth_wide)
    plt.plot(x_r_nosys_nosysimpl, f_deltaNLL_nosys_nosysimpl(x_r_nosys_nosysimpl), color='C0', lw=linewidth_wide)
    vscale = 1 / y_limit[1]
    plt.plot([x_limit[0], 1 - constraints_sys_nosysimpl[0]], [1, 1], 'k', lw=linewidth_narrow)
    plt.plot([1 + constraints_sys_nosysimpl[1], x_limit[1]], [1, 1], 'k', lw=linewidth_narrow)
    plt.plot([x_limit[0], 1 - constraints_nosys_nosysimpl[0]], [1, 1], 'k', lw=linewidth_narrow)
    plt.plot([1 + constraints_nosys_nosysimpl[1], x_limit[1]], [1, 1], 'k', lw=linewidth_narrow)
    plt.axvline(x=1 - constraints_sys_nosysimpl[0], ymax=1. * vscale, color='C3', lw=linewidth_narrow)
    plt.axvline(x=1 + constraints_sys_nosysimpl[1], ymax=1. * vscale, color='C3', lw=linewidth_narrow)
    plt.axvline(x=1 - constraints_nosys_nosysimpl[0], ymax=1. * vscale, color='C0', lw=linewidth_narrow)
    plt.axvline(x=1 + constraints_nosys_nosysimpl[1], ymax=1. * vscale, color='C0', lw=linewidth_narrow)
    plt.plot([0], [0], color='C3', label="$\mu_{}$ = 1.00 (-{:.3f} +{:.3f})".format('\mathrm{stat. + sys.}', constraints_sys_nosysimpl[1], constraints_sys_nosysimpl[0]))
    plt.plot([0], [0], color='C0', label="$\mu_{}$         = 1.00 (-{:.3f} +{:.3f})".format('\mathrm{stat.}', constraints_nosys_nosysimpl[1], constraints_nosys_nosysimpl[0]))
    ...
    plt.ylabel("-2 $\cdot \/ \Delta$NLL")
    plt.ylim((y_limit[0], y_limit[1]))
    plt.plot(x_r_sys_sysimpl, f_deltaNLL_sys_sysimpl(x_r_sys_sysimpl), color='C3', lw=linewidth_wide)
    plt.plot(x_r_nosys_sysimpl, f_deltaNLL_nosys_sysimpl(x_r_nosys_sysimpl), color='C0', lw=linewidth_wide)
    vscale = 1 / y_limit[1]
    plt.plot([x_limit[0], 1 - constraints_sys_sysimpl[0]], [1, 1], 'k', lw=linewidth_narrow)
    plt.plot([1 + constraints_sys_sysimpl[1], x_limit[1]], [1, 1], 'k', lw=linewidth_narrow)
    plt.plot([x_limit[0], 1 - constraints_nosys_sysimpl[0]], [1, 1], 'k', lw=linewidth_narrow)
    plt.plot([1 + constraints_nosys_sysimpl[1], x_limit[1]], [1, 1], 'k', lw=linewidth_narrow)
    plt.axvline(x=1 - constraints_sys_sysimpl[0], ymax=1. * vscale, color='C3', lw=linewidth_narrow)
    plt.axvline(x=1 + constraints_sys_sysimpl[1], ymax=1. * vscale, color='C3', lw=linewidth_narrow)
    plt.axvline(x=1 - constraints_nosys_sysimpl[0], ymax=1. * vscale, color='C0', lw=linewidth_narrow)
    plt.axvline(x=1 + constraints_nosys_sysimpl[1], ymax=1. * vscale, color='C0', lw=linewidth_narrow)
    plt.plot([0], [0], color='C3', label="$\mu_{}$ = 1.00 (-{:.3f} +{:.3f})".format('\mathrm{stat. + sys.}', constraints_sys_sysimpl[1], constraints_sys_sysimpl[0]))
    plt.plot([0], [0], color='C0', label="$\mu_{}$         = 1.00 (-{:.3f} +{:.3f})".format('\mathrm{stat.}', constraints_nosys_sysimpl[1], constraints_nosys_sysimpl[0]))
    plt.legend(bbox_to_anchor=(0., 1.02, 1., .102), loc=3, ncol=1, mode="expand", borderaxespad=0., prop={'size': 14})

...
def build_dataset(path, classes, fold, make_categorical=True, use_class_weights=True):
    ...
    logger.info("\n----------------------------------------\nInput (width, height) before stacking: {}, {}".format(len(xs[0][0]), len(xs[:])))

    # Stack inputs
    xs = np.vstack(xs)
    logger.debug('Input dataset (shape): {}'.format(xs.shape))
    logger.info("\n----------------------------------------\nInput after stacking: {}".format(xs[0][0]))
    logger.info("\n----------------------------------------\nInput (width, height) after stacking: {}, {}".format(len(xs[0][0]), len(xs[:])))

...
def main(args):
    ...
    for i, (up, down) in enumerate(zip(bins[1:], bins[:-1])):
        ...
        procs_up['qcd'] = tf.maximum(procs['qcd'] * met_upshift_ph, 0)
        procs_down['qcd'] = tf.maximum(procs['qcd'] * met_downshift_ph, 0)
    ...
    bincontent_nom_, bincontent_up_, bincontent_down_ = session.run([bincontent_nom, bincontent_up, bincontent_down], \
                        feed_dict={x_ph: x_preproc, y_ph: y, w_ph: w, scale_ph: fold_factor, met_downshift_ph: met_downshift, met_upshift_ph: met_upshift})
